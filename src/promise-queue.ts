...
// A function that produces a Promise object
type Generator<T> = (...args:Array<any>) => Promise<T>

// Interface for queue elements, which store a function and its arguments that are evaluated
// when the particular element is retrieved from the front of the queue
 ...
 */
export const PromiseQueueEmptyError = {
...
export class PromiseQueue<T> {
	...
	// The underlying array of function/argument pairs
	private _array: Array<PromiseQueueItem<T>>
	 ...
	 */
	async element(): Promise<T> {
		const el = await this.peek()
		if (el == null)
			throw PromiseQueueEmptyError
		return el
	}
	 ...
	 */
	async batch(size: number): Promise<Array<T>> {
	 ...
	 * @returns {number} index to start splicing from
	 */
	private nextBatchIndex(size: number): number {
